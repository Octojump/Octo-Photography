...
// ----------------- SITE MESSAGE -----------------
const siteMessageModal = document.createElement("div");
siteMessageModal.id = "site-message-modal";
siteMessageModal.style = "display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:10001; justify-content:center; align-items:center; flex-direction:column;";
siteMessageModal.innerHTML = `
  <div style="background:#222; padding:20px; border-radius:15px; width:300px; max-width:90%; text-align:center; color:white; position:relative;">
    <span id="site-message-close" style="position:absolute; top:10px; right:15px; font-size:25px; cursor:pointer;">&times;</span>
    <div id="site-message-content" style="margin-top:15px; font-size:1.1rem;"></div>
    <button id="site-message-ok" style="margin-top:15px; padding:8px 12px; border:none; border-radius:8px; background:#ff7b00; color:white; cursor:pointer;">OK</button>
  </div>`;
document.body.appendChild(siteMessageModal);
...
if (loginAdminBtn) {
  loginAdminBtn.addEventListener("click", () => {
    ...
    if (pass === ADMIN_CODE) {
      ...
      showSiteMessage("✅ Admin connecté");
      updateAdminLists();
    } else {
      showSiteMessage("❌ Code admin incorrect");
    }
  });
}
...
function renderGallery() {
  ...
  displayPhotos.forEach(p => {
    ...
    likeBtn.addEventListener("click", (e)=>{
      e.stopPropagation(); // empêche l'ouverture du lightbox
      toggleLike(p.id);
    });
  ...
  if (displayPhotos.length === 0) {
    ...
    empty.textContent = "Aucune photo pour l'instant."; 
...
function openLightbox(idx) {
  if (idx < 0 || idx >= photos.length) return;
  const p = photos[idx]; photos[idx].views++;
...
function resetGallery() {
  // Réinitialise les vues et likes pour chaque photo
  photos = photos.map(p => ({ ...p, views: 0 }));

  // Réinitialise complètement l'objet likes et userLikes
  likes = {};
  userLikes = [];
  ...
  // Sauvegarde dans le localStorage pour écraser les anciennes données
  localStorage.setItem("photos", JSON.stringify(photos));
  localStorage.setItem("likes", JSON.stringify(likes));
  localStorage.setItem("userLikes", JSON.stringify(userLikes));

  // Re-render la galerie et les boutons
...
function updateAdminLists() {
  ...
  if (photoSelect) { 
    ...
    photos.forEach((p,i)=>{ 
      ...
      opt.value = i; 
      opt.textContent = `${p.alt} (${p.category})`; 
